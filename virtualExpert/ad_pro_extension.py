...
from virtualExpert import ad_pro_serializer
import random
import string
import yagmail
...
def validate_email(id):
    allData = models.ad_provider.objects.all()
    profile = ad_pro_serializer.adproviderSerializer(allData,many=True)
    #users
    usersData = models.users.objects.filter(work = "ad_provider").values()
    data = False
...
def validate_otp(id, otp):
    specificUserData = models.ad_provider.objects.get(uid = id)
    data = ad_pro_serializer.adproviderSerializer(specificUserData)
    valid = False
...
def verify_user(email, password):
    try:
        specificData = models.ad_provider.objects.get(email = email)
        data = ad_pro_serializer.adproviderSerializer(specificData)
...
def verify_forget_otp(id):
    try:
        specificData = models.ad_provider.objects.get(uid = id)
        data = ad_pro_serializer.adproviderSerializer(specificData)
        authentication = False
