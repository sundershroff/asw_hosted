...
def hm_upload_account(request,id):
    try:
        ...
        # path = fs.save(f"virtual_expert/hiring_manager/{id}/id_card/"+id_card, request.FILES['id_card'])
...
def ad_provider_upload_account(request,id):
    try:
        ...
        if basicdetailsserializer.is_valid():
            ...
            ad_provider = jsondec.decode(hm_data['ad_provider']) 
            alter_values = []
            print("new")
            print(get_uid['uid'])
            for x in ad_provider:
                if get_uid['uid'] == x['uid']:
                    alter_values.append(get_uid)
                else:
                    alter_values.append(x)
...
def ad_distributor_upload_account(request,id):
    try:
        ...
        if basicdetailsserializer.is_valid():
            ...
            ad_distributor = jsondec.decode(hm_data['ad_distributor']) 
            alter_values = []
            print("new")
            print(get_uid['uid'])
            for x in ad_distributor:
                if get_uid['uid'] == x['uid']:
                    alter_values.append(get_uid)
                else:
                    alter_values.append(x)
...
def hm_password_reset(request,id):
    try:
        ...
        content = f"""
        PasswordResetform : {f"http://localhost:8001/hm_password_reset/{id}"}
        """
